...
module.exports.handleEvent = async function({ event, api, Users, Threads }) {
  try {
    ...
    if (!sData.threads[threadID]) {
      ...
      sData.threads[threadID] = { name: tname, total: 0, daily: {}, users: {} };
    } else {
...
function addNoise(ctx, x, y, w, h, amount = 3500) {
  ...
  for (let i=0;i<amount;i++){
    ...
    const a = Math.random()*0.06; // very subtle
...
// Chart (monthly): Line=Message (green), Bars=Reply (pink)
function drawChart(ctx, x, y, w, h, labels, lineVals, barVals) {
  ...
  const maxBar = barVals ? Math.max(1, ...barVals) : 1;
  ...
  for (let i=0;i<n;i++){
    ...
    const bx = n < 2 ? px + (pw - barW) / 2 : px + i*step + (step-barW)/2;
    const by = py + ph - bh;
  ...
  const pts = lineVals.map((v,i)=>({
    xx: n < 2 ? px + pw / 2 : px + (pw*i)/maxTicks,
  ...
  for (let i=0;i<n;i++){
    ...
    const xx = n < 2 ? px + pw / 2 : px + (pw*i)/maxTicks;
...
module.exports.run = async function({ api, event, Users, Threads }) {
  ...
  // Top 3 commands used by this user (ALL time)
  const topCmds = Object.entries(u.cmdByName || {})
  ...
  // Top users in current thread (bxh 6)
  const topUsers = Object.entries(threadStats.users || {})
    .map(([id, total]) => ({
      id,
      name: sData.users?.[id]?.name || id,
  ...
  const msgSeries = extendMonthlySeries(buildMonthlySeries(sData.daily, startDay, new Date()), 6);
  ...
  // Top commands by user (TOP 3 lệnh cụ thể)
  card(ctx, 970, 170, 522, 250);
  const topCmdHeader = fitText(ctx, "Top lệnh người chơi đã xài", 420, 22, "900");
  txt(ctx, topCmdHeader.text, 1000, 215, topCmdHeader.size, "rgba(232,234,237,0.75)", "900");
  txt(ctx, "</>", 1450, 215, 26, "rgba(232,234,237,0.7)", "900", "right");

  for (let i=0;i<3;i++){
    const c = topCmds[i] || { name: "N/A", count: 0 };
    const y = 240 + i*70;

    pill(ctx, 1000, y, 470, 55);
    // rank
    pill(ctx, 1010, y+10, 44, 35);
    txt(ctx, String(i+1), 1032, y+34, 18, "#e8eaed", "900", "center");

    // cmd name
    pill(ctx, 1060, y+10, 250, 35);
    const cmdFit = fitText(ctx, c.name, 240, 18, "900");
    txt(ctx, cmdFit.text, 1185, y+34, cmdFit.size, "#e8eaed", "900", "center");

    // count
    pill(ctx, 1318, y+10, 142, 35);
    const countFit = fitText(ctx, `${formatComma(c.count)} lần`, 130, 18, "900");
    txt(ctx, countFit.text, 1389, y+34, countFit.size, "rgba(232,234,237,0.9)", "900", "center");
  }

  // ===== Row 2 =====
  // Top server users
  card(ctx, 40, 440, 760, 270);
  const topUserHeader = fitText(ctx, "Top server Tương Tác Của tất cả Người Chơi", 680, 22, "900");
  txt(ctx, topUserHeader.text, 70, 485, topUserHeader.size, "rgba(232,234,237,0.75)", "900");
  const chartIcon = await fetchIcon("chart", "📈", 26);
  if (chartIcon) ctx.drawImage(chartIcon, 758, 466, 24, 24);
  ...
  for (let i=0;i<6;i++){
    ...
    pill(ctx, 120, rowY, 420, 26);
    const userRowFit = fitText(ctx, uu.name, 400, 16, "900");
    txt(ctx, userRowFit.text, 140, rowY+20, userRowFit.size, "#e8eaed", "900");

    pill(ctx, 550, rowY, 230, 26);
    txt(ctx, `${formatShort(uu.total)} msg`, 665, rowY+20, 16, "rgba(232,234,237,0.85)", "900", "center");
  }

  // Charts
  card(ctx, 820, 440, 672, 270);
  txt(ctx, "Charts", 850, 485, 28, "rgba(232,234,237,0.75)", "900");

  dot(ctx, 1150, 478, "rgba(90,200,120,0.95)");
  txt(ctx, "Message", 1170, 485, 22, "rgba(232,234,237,0.75)", "900");

  dot(ctx, 1310, 478, "rgba(220,90,160,0.95)");
  txt(ctx, "Reply", 1330, 485, 22, "rgba(232,234,237,0.75)", "900");

  // chart draw (reply bars pink)
  drawChart(ctx, 835, 505, 652, 190, msgSeries.labels, msgSeries.values, replySeries.values);
  ...
  return api.sendMessage(
    { body: "📊 Thống kê toàn bộ server", attachment: fs.createReadStream(outPath) },
    threadID
  );
};
